...
2. Бот берёт первый вопрос и назначает «называющего» (первого игрока). Один вопрос за запуск — ротации нет.
3. Называющий пишет свой процент (карточка с полем, в личку).
4. Бот оглашает число называющего в группе, остальные голосуют **Higher / Lower** (кнопки в группе).
