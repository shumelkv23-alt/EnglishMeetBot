...
from app.services.games.banks.two_truths_bank import random_two_truths
from app.services.games.banks.crossword_bank import random_crossword_words
...
def _sanitize_word(value) -> str:
    """Привести слово из ответа LLM к виду для кроссворда (3–8 латинских букв)."""
    word = "".join(ch for ch in str(value or "").lower() if ch.isascii() and ch.isalpha())
    return word if 3 <= len(word) <= 8 else ""


async def generate_crossword_words(count: int = 8) -> list[dict]:
    """Один вызов LLM: слова + короткие подсказки для кроссворда.

    Возвращает список `[{'word': str, 'clue': str}, ...]`. Слова — обычные
    английские (3–8 букв), с общими буквами, чтобы локальный решатель мог их
    пересечь. При сбое LLM или невалидном ответе — детерминированный банк.
    """
    settings = _settings()
    model = settings.llm_games_model
    payload = {
        "model": model,
        "max_tokens": 2048,
        "system": "You create English crossword puzzles. ANSWER ONLY WITH JSON.",
        "messages": [
            {
                "role": "user",
                "content": (
                    f"Create {count} common English words for a crossword (3-8 letters, "
                    "lowercase). Choose words that share common letters (a, e, t, r, s, o, n, l) "
                    "so they can intersect. For each give a short simple clue in English "
                    "(3-8 words). Return strict JSON of the form "
                    '{"words": [{"word": "cat", "clue": "A small furry pet"}, ...]}.'
                ),
            }
        ],
    }
    ...
    if content:
        ...
        for item in data.get("words") or []:
            if not isinstance(item, dict):
                continue
            word = _sanitize_word(item.get("word"))
            clue = str(item.get("clue") or "").strip()
            if word and clue:
                items.append({"word": word, "clue": clue})
        if len(items) >= 4:
            return items[:count]
    return random_crossword_words(count)
