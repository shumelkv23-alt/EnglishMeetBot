"""Игра Snake Oil / «Snake Oil» 🧪 — питчинг абсурдных товаров.
...
from app.services.games.banks.snake_bank import random_persona, random_problem
from app.services.weekly_poll import get_or_create_config
from app.services.games.banks.word_bank import sample_product_words
...
WINNER_PHRASES = [
    "A genius seller — the customer is already reaching for their wallet! 💸",
    "Only Snake Oil legends sell like that! 🧪",
    "Brilliant pitch — the product flew off the shelves! 🔥",
    "The customer couldn't resist — and we get it! 😎",
    "A true master of the pitch! 👑",
]
...
def build_scoreboard_card(
    ...
    if game.status == "setup":
        subtitle = "Press 'I'm in!' then 'Start'"
    elif game.status == "finished":
        subtitle = "Game over"
    else:
        subtitle = f"First to {game.target_score} points"

    lines = [f"{_display(prof)} — {sp.score} pts" for sp, prof in players] or ["No one is playing yet"]
...
def build_seller_card(round: SnakeRound, word1: str, word2: str, action_url: str, solo: bool = False) -> dict:
    ...
    sections = [
        {"widgets": [{"textParagraph": {"text": (
            ...
            f"Your product from these words:\n🔹 {word1}\n🔹 {word2}\n\n"
            "Come up with a product and pitch it!"
        )}}]},
    ]
    if solo:
        sections.append({"widgets": [{"buttonList": {"buttons": [
            {"text": "🎤 Ready — judge me", "onClick": {"action": {
                "function": action_url,
                "parameters": [
...
def build_customer_card(
    ...
    return {"cardsV2": [{
        ...
        "card": {
            ...
            "sections": [
                {"widgets": [{"textParagraph": {"text": "Who pitched the best product?\n\n" + "\n".join(lines)}}]},
...
def build_round_card(
    ...
    if solo:
        ...
        hint = "Come up with a product and press 'Ready' in DMs 🎤"
    else:
...
def _judge_pair(
    ...
    system = (
        "You're the customer in 'Snake Oil'. Your persona and problem are given. "
        "You're offered two products, each built from two random words. Pick the more "
        'funny/clever product. Return strict JSON of the form {"winner": 1}, where 1 or 2 is '
        "the product number. No comments or markup."
    )
    user_prompt = (
...
async def _judge_round(
    ...
    """Choose победителя в solo-раунде: LLM между парой игрока и парой бота."""
...
async def start_game(db: AsyncSession, game_id: int) -> dict:
    ...
    if game.status != "setup":
        return {"ok": False, "text": "Game already started 🚀"}
    players = await _players_of_game(db, game_id)
    if not _is_solo(players) and len(players) < 3:
        return {"ok": False, "text": "Need at least 3 players 🙏"}
...
async def _finalize_vote(db: AsyncSession, round: SnakeRound, game: SnakeGame, winner: Profile) -> dict:
    ...
    await _refresh_scoreboard(db, game)
    return {"ok": True, "cards_v2": build_done_card("Next round started — check your DMs 😉")["cardsV2"]}
