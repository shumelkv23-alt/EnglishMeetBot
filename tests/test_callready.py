...
def test_module_points():
    ...
    st["grammar"] = {"0": True, "1": True, "2": True}
    assert callready.module_points(st) == 2 * 10 + 3 * 1
...
def test_progress_summary_reflects_state():
    ...
    assert "Block 1 · Call foundations: 4/4" in summary
    assert "40/40" in summary
    assert "5/5" in summary
    assert str(4 * 10 + 40) in summary
...
def test_all_cards_share_single_card_id():
    ...
    assert _card_id(callready.build_feedback_card(0, 0, True, task, 1)) == "callready"
...
def test_feedback_card_last_question_has_no_next():
    ...
    card = callready.build_feedback_card(topic, last_pos, True, task, 1)
    widgets = card["cardsV2"][0]["card"]["sections"][0]["widgets"]
